...
let rightSmaller = (arr) => {
  ...
  for (let i = arr.length - 1; i >= 0; i--) {
    ...
    if (stack.length != 0 && arr[stack[stack.length - 1]] < arr[i]) {
      ansList2[i] = arr[stack[stack.length - 1]];
    } else {
      ansList2[i] = -1;
    }
    stack.push(i);
...
leftSmaller([39, 27, 11, 4, 24, 32, 32, 1]);
rightSmaller([39, 27, 11, 4, 24, 32, 32, 1]);
console.log("asnListasnList", asnList, ansList2);
